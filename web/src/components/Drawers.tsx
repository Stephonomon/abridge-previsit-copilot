...
export function VersionsDrawer({
  ...
  return (
    <Drawer title="Agent versions" subtitle="Different prompt versions for different patient populations" onClose={onClose}>
      <div className="space-y-3">
        {config.versions.map((v) => {
          const active = v.id === config.activeVersionId;
          return (
            <button
              key={v.id}
              onClick={() => onActivate(v.id)}
              className={`w-full text-left border rounded-xl p-4 transition-colors ${
                active ? "border-indigo-brand ring-1 ring-indigo-brand/30 bg-indigo-50/40" : "border-stone-200 hover:border-stone-300"
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="font-bold text-sm">{v.id}</span>
                <span className="text-sm text-stone-600">{v.label}</span>
                {active && (
                  <span className="ml-auto text-[11px] font-bold text-indigo-brand-dark bg-indigo-soft rounded-full px-2 py-0.5">ACTIVE</span>
                )}
              </div>
              <div className="text-xs text-stone-500 mt-1">
                {v.customizations.length === 0
                  ? "Base scaffold — no customizations"
                  : `${v.customizations.length} customization${v.customizations.length === 1 ? "" : "s"}`}
              </div>
            </button>
          );
        })}
